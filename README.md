...
秉承“Better Experience更优体验”的原则，在继承原版优秀界面的基础上，深度优化了诸多细节功能与预置配置，致力于成为体验更好的 Mihomo 客户端。
...
### 💎 深度体验优化
* **开箱即用**：自动化的权限处理与舒适的TUN&VPN体验，预置更加适合中国用户的内核配置，减少繁琐的手动调试。
* **精雕细琢**：重新审视打磨每一处细节，稳定无感的轻量模式，移动端更加省电、桌面端更低占用、旨在提供更好的Mihomo客户端体验。

### 🛡️ 安全与稳定性
* **安全校验**：遵循Mihomo官方安全建议/内核升级紧跟正式版，同时桌面端有严格的安全校验以及权限控制，有效防范潜在的TOCTOU或非法访问，多平台增加了配置文件改动错误的优雅回退机制。
* **高稳定性**：通过了作者本身和群友长期以来高强度的压力测试与后台测试，优化了多处极端场景下的使用稳定性问题。

### 🎨 自由可定制化
* **可视化管理**：更加全面且易用的内核配置界面，支持更多参数的可视化操作调整和实时生效。
* **实用小组件**：提供多种美观实用风格的 Widgets，在首页可以掌控全局流量和当前运行状态。
* **个性化定制**：丰富的色彩主题、自定义图标/首页标题与界面布局调整，让每个人都可以拥有自己独一无二的Bettbox。

### 💻 多个平台支持
* **广泛兼容**：支持主流架构，为现代设备额外优化的性能(例如桌面端CPU分级和Android 16K对齐)，同时针对多平台 Desktop ARM64 设备提供了原生适配。
...
* [FlClash](https://github.com/chen08209/FlClash) - 来自陈师傅的优秀项目。
